...
function init()
  connect(g_game, {
	...
	connect(g_game, 'onTextMessage', OffChat),
    onGameStart = online,
    onGameEnd = offline
  })
...
function OffChat(mode, text)
	if not g_game.isOnline() then
		return
	end
	if mode == MessageModes.Failure then
		if string.find(text, "ChatOffline") then
			disableChat()
			consoleToggleChat:disable()
		elseif string.find(text, "ChatOnline") then
			enableChat()
			consoleToggleChat:enable()
		elseif string.find(text, "LimparChat") then
